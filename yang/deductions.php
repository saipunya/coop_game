...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
  try {
    ...
    if ($action === 'delete') {
      if (!user_can('deductions_delete', $user)) throw new RuntimeException('บัญชีนี้ไม่มีสิทธิ์ลบรายการหัก');
      if (!$id) throw new RuntimeException('ไม่พบรายการที่ต้องการลบยอดหัก');
      db()->beginTransaction();
      $stmt = db()->prepare('SELECT * FROM tbl_rubber_workflow WHERE workflow_id = :id FOR UPDATE');
      $stmt->execute(['id' => $id]);
      $workflow = $stmt->fetch();
      if (!$workflow || !in_array($workflow['workflow_status'], ['deducted', 'paid'], true)) throw new RuntimeException('รายการนี้ยังไม่มีข้อมูลยอดหักให้ลบ');
      if ($workflow['workflow_status'] === 'paid' && !$isAdmin) throw new RuntimeException('รายการที่จ่ายเงินแล้วลบยอดหักได้เฉพาะ admin');
      db()->prepare('DELETE FROM tbl_rubber_deduction WHERE workflow_id = :id')->execute(['id' => $id]);
      $stmt = db()->prepare('UPDATE tbl_rubber_workflow SET price_per_kg = 0, gross_amount = 0,
        total_deduction = 0, net_amount = 0, workflow_status = "weighed",
        deduction_by = "", deduction_at = NULL, receipt_no = NULL,
      ...
      audit_log('delete', 'workflow', $id, 'ลบรายการหักและย้อนกลับไปสถานะชั่งแล้วของสมาชิก ' . $workflow['member_number'], [
        'round_date' => $workflow['weigh_date'], 'yard_code' => $workflow['yard_code'],
        'previous_status' => $workflow['workflow_status'], 'previous_total_deduction' => (float) $workflow['total_deduction'],
        'previous_net_amount' => (float) $workflow['net_amount'], 'previous_receipt_no' => $workflow['receipt_no'],
      ]);
      db()->commit();
      $_SESSION['workflow_flash'] = ['type' => 'success', 'message' => 'ลบรายการหักและข้อมูลการจ่ายเงินที่เชื่อมกันแล้ว รายการย้อนกลับไปรอบันทึกยอดหัก'];
      workflow_redirect('deductions.php');
    }
    ...
    if ($workflow['workflow_status'] === 'deducted' && !user_can('deductions_edit', $user)) {
      throw new RuntimeException('บัญชีนี้ไม่มีสิทธิ์แก้ไขรายการหักที่บันทึกแล้ว');
    }
